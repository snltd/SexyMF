...
function smfError(req, res, next) {
	...
	return {
		...
		unknownCmd: function(cmd) {
			doError('Unknown or incorrectly formed command: ' + cmd,
		...
		unknownState: function(state) {
			doError('Unknown state: ' + state, 'InternalError');
		},

		ngz2ngz: function() {
			doError('NGZ to NGZ unsupported', 'InvalidArgumentError');
		},

		disallowedCmd: function(cmd) {
			doError('execution of \'' + cmd + '\' disallowed or restricted',
					'InternalError');
		},
		...
		unauthorizedCmd: function(auth) {
			doError('command requires \'' + auth + '\' auth',
							'NotAuthorizedError', 'insufficient authorization');
		},
	...
	function doError(log_str, rest_err, rest_str) {
		...
		// Here's where we do the error handling. Write something to the log
		// file, and send the REST error.
		//
		smfCore.log('err', 'client_msg=' + log_str);
