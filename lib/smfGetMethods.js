...
svcpropCmd: function(req, res, next) {
  ...
  var cmd = smfCommand(['/bin/svcprop', req.params.svc], req.params.cache,
      'view', eh);
...
svccfgCmd: function(req, res, next) {
  ...
  // export requires a service, extract and archive don't.  Archive
  ...
  // Solaris will be using zlogin if it's an NGZ. Illumos may or may
  // not be.
  ...
  if (smfCommon.illumos_ngz(req.params.cache)) {
    ...
    var input = 'repository ' + smfCommon.path_to_db(req.params) + '\n';

    if (c === 'extract' || c === 'export' || c === 'archive') {
      input += c;

      if (c === 'export') {
        input += ' ' + req.params.svc;
      }
...
fetchLog: function(req, res, next) {
  ...
  if (req.params.cache.p.illumos) {
    cmd = smfCommand(['/bin/svcs', '-L', req.params.svc], req.params.cache,
        auth, eh);
    ...
    cmd.cmdExec(function(err, stdout, stderr) {
      ...
      // The next external will be tail(1), accessing a file in the root
      // of another zone. So, we don't want to add in the '-z zone' part
      // of the command - it will make no sense. If we remove reference
      // to the other zone, smfCommand will build a normal "local"
      // command.
...
showStatus: function(req, res, next) {
  ...
  if (smfConfig.show_status) {
    smfLog.debug('sent application status to user');
    ...
    res.send(
      {
        sexymf: {
          ...
          api_calls: req.params.cache.p.requests,
          config_files: smfConfig.c_paths,
          logs: {
            file: smfConfig.log.logfile,
    ...
    smfLog.info('user requested status, but show_status is false');
  }

},


svccfgSupports: function(req, res, next) {
  //
  // Send the user a JSON object detailing the commands and sub-commands
  // this version of svccfg(1m) supports.
  //
  if (smfConfig.show_status) {
    smfLog.debug('sent svccfg help to user');

    exec('/usr/sbin/svccfg help', function(err, stdout, stderr) {
      var i,
          to_send = {},
          chunks,
          rows;
      ...
      for(i = 0; i < rows.length; i++) {
        //
        // The first line contains 'svccfg:' for some reason.
        //
        if (i === 0) {
          rows[i] = rows[i].replace(/^svccfg: /, '');
        }

        chunks = rows[i].split(/:\s+/);

        if (chunks.length == 2) {
          to_send[chunks[0]] = chunks[1].split(/\s/);
        }
    ...
    smfLog.info('user requested svccfg help, but show_status is false');
  }
