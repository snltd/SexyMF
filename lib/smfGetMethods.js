...
svcsCmd: function(req, res, next) {
	...
	if (req.params.svc) {
		...
		cmd.cmdExec(function(err, stdout, stderr) {
			...
			// Given that this only takes a service as an argument, let's
...
svccfgCmd: function(req, res, next) {
	...
	proc = smfCommand(args, req.params.cache, auth, eh).cmdSpawn();

	if ( (req.params.cache.t.zone !== '@' && req.params.cache.t.zone !==
				req.params.cache.p.zonename) && req.params.cache.p.illumos) {

		// Work out the full path to the zone's repository file

		var db = path.join(req.params.cache.p.zpaths[req.params.zone], 'root',
				'etc', 'svc', 'repository.db');

		var input = 'repository ' + db + '\n' + c + '\n\0';

		smfCore.log('notice', 'svccfg input=\'' + input.replace(/\n/g, '\\n') +
				'\'');

		proc.stdin.write(input);
	}

	stream_handler(eh, cmd, 'application/xml', proc, req, res);
},
...
fetchLog: function(req, res, next) {
	...
	function process_log_file(path) {
		...
		stream_handler(eh, logcmd, 'text/plain', logproc, req, res);
	}
...
function stream_handler(eh, cmd, type, proc, req, res) {
	...
	proc.stdout.on('data', function(data) {
		...
		// before here, Restify seems to get confused about the content-type to
		// use for errors.
	...
	// be a non-existent service though.

	proc.stderr.on('data', function(data) {
		smfCore.log('err', 'stderr: ' + data);

		// Remember that data comes in as a buffer.

		if (data.toString().match(/Permission denied/)) {

			// can't read repository.db on NGZ svccfg command

			eh.genericErr('cannot read db file');
		}
		else {
			eh.unknownSvc(req.params.svc);
		}
