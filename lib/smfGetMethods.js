...
svcpropCmd: function(req, res, next) {
	...
	var cmd = core.mk_cmd( [ '/bin/svcprop', req.params.svc ], req, next );
...
svccfgCmd: function(req, res, next) {
	...
	// is the proper, asynchronously asynchrous way to do this.)
...
function svcState(req, res, next) {
	...
	var out = [],
		cmd = core.mk_cmd( [ '/bin/svcs', '-H', req.params.svc ], req,
				next);
