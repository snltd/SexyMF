...
svccfgCmd: function(req, res, next) {
	...
	if (cmd.isAllowed(to_run)) {

		if (cmd.isAuthorized()) {
			proc = spawn(to_run, cmd_arr);
		}
		else {
			eh.unauthorizedCmd(cmd.asStr, auth);
		}

	}
	else {
		eh.disallowedCmd(to_run);
	}
...
fetchLog: function(req, res, next) {
	...
	if (req.params.cache.p.illumos) {
		...
		cmd.cmdExec(function(err, stdout, stderr) {
			...
			return process_log_file(path.join('/var/svc/log',
							svcs[0].replace(/^.*svc:\//, '').replace(/\//g, '-') + '.log'),
							req, res, next);
		});
