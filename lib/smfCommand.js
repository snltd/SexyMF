...
function smfCommand(raw_cmd, cache, auth, eh) {
	...
	return {
		...
		cmdSpawn: function(callback) {
			...
			smfCore.log('debug', 'spawning: ' + cmd_str);
			var external = exec_arr.shift();
			var proc = spawn(external, exec_arr);
			smfCore.log('debug', 'process ' + proc.pid + ' launched');
			return proc;
		},

		logSpawnInput: function(cmd, input) {
										 //
			// Log the input that's sent to a spawned command

			smfCore.log('notice', cmd + 'input=\'' + input.replace(/\n/g, '\\n') +
					'\'');
		},

		logSpawnStdErr: function(data) {
			smfCore.log('err', cmd_word +' STDERR: ' + data.toString());
		},

		logSpawnClose: function(pid, exit) {
			smfCore.log('notice', 'process ' + pid + ' exit code: ' + exit);
		}
