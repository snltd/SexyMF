...
function smfCommand(raw_cmd, cache, auth, eh) {
  ...
  // auths. If not, there's no point going any further. We might not
  // even need to check for auths. The access list middleware doesn't.
  ...
  if (auth) {
    ...
    if (_.contains(cache.t.user.auths, auth)) {
      ...
      smfLog.info({required: auth, user: cache.t.user.name, auths:
                  cache.t.user.auths.join(',')}, 'authorization failed');

      return eh.unauthorizedCmd(auth);
    }
  ...
  if (runInOtherZone() && cache.t.z_method === 'zlogin') {
  ...
  if (runInOtherZone() && cache.t.z_method === 'illumos' &&
      cmd_word !== '/usr/sbin/svccfg') {
  ...
  if (cache.t.subcmd) {
    ...
    if (_.contains(_.keys(eb[cmd_word]), cache.t.subcmd)) {
      ...
      return eh.disallowedSubCmd(cache.t.subcmd);
    }
  ...
  // If this is a svccfg command for another zone, on Illumos, our
  // command becomes svccfg reading input from stdin -- UNLESS it's
  // import. The calling routine will write to its stdin. This has to be
  // spawn()ed, but all our svccfg commands are anyway.

  if (cmd_word === '/usr/sbin/svccfg' && runInOtherZone() &&
      cache.t.z_method === 'illumos') {
  ...
  return {
    ...
    stream_handler: function(type, proc, req, res, send_complete) {
      ...
      proc.stderr.on('data', function(data) {
        //
        // We now log any stdout as a notice, but don't halt execution.
        // We also store it in a buffer, to be examined at process end.
        //
        all_stderr += data.toString();
      ...
      proc.on('close', function(exit_code) {
        ...
        if (exit_code === 0) {
          ...
          //
          // The last thing written to stderr is in 'all_stderr'. We can
          // examine that to see what message to send to the user. I don't
          // like this, because it's not locale independent.

          if (all_stderr.match(/doesn't match any services/)) {
            return eh.unknownSvc(req.params.svc);
          }
          else if (all_stderr.match(/Unknown command/)) {
            return eh.unknownCmd(req.params.cmd);
          }
          else if (all_stderr.match(/couldn't parse/)) {
            return eh.invalidDocument();
          }
          else {
            return eh.genericErr('Command did not complete', all_stderr);
          }
  ...
  function cmdLog(err) {
    ...
    // Log information about any external commands being executed.
