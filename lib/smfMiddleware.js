...
mkParams: function(req, res, next) {

	// In certain routes have to match service names with a regex, because
	// using :svc will choke on the slashes and :s in FMRIs. This middleware
	// puts the regex matches into the variables that everything else uses.
	// It has to be used BEFORE the 'chk' methods.
	
	if (req.params[0]) {
		req.params.zone = req.params[0];
	}

	if (req.params[1]) {
		req.params.svc = req.params[1];
	}
...
chkCmd: function(req, res, next) {
	...
	if (cmd && (cmd.length > 20 || cmd.match(/[^a-z]/))) {
		return next(new restify.InvalidArgumentError('invalid command ' +
					cmd));
	}
...
chkType: function(req, res, next) {
	...
	// isn't complete junk, it's up to the user to supply something
	// sensible. Types can only have lower case letters

	var cmd = req.params.type;

	if (type && (type.length > 20 || type.match(/[^a-z]/))) {
		return next(new restify.InvalidArgumentError('invalid datatype ' +
					type));
	}
...
chkFlag: function(req, res, next) {
	...
	if (flags && (flags.length > 10 || flags.match(/[^a-z]/))) {
		return next(new restify.InvalidArgumentError('invalid flag(s) ' +
					flags));
	}

	// If there are flags, prefix them with a '-', if not, set the variable
	// to a blank string.
