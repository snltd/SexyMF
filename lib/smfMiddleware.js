...
accessZone: function(req, res, next) {
	...
	if (acl[zone]) {
		acc_obj = acl[zone];
		console.log('found acl for zone ' + zone);
	}
	else if (acl['default']) {
		acc_obj = acl['default'];
		console.log('using default acl');
	}
	else {
		return next();
	}
	...
	if (req.params.cache.p.fmri[req.params.svc]) {
		...
		cmd.cmdExec(function(err, stdout, stderr) {
			...
			// Pull out the FMRI and stick it in the cache

			req.params.cache.p.fmri[req.params.svc] = svcs[0].split(/\s+/)[2];
			check_fmri();
		});
	...
	function check_fmri() {
		...
		//
		var allow = false,
				fmri = req.params.cache.p.fmri[req.params.svc];
