		...
		exec('/usr/sbin/zoneadm -z ' + zone + ' list', function(err, stdout,
			...
			if (err) {
				...
				if (req.params.cache.p.illumos) {
					...
					// We'll need the zonepath if the user issues svccfg. Get it now
					// and cache it.

					if (req.params.cache.p.zpaths[zone]) {
						smfLog.debug('zonepath in cache');
						return next();
					}
					else {

						exec('/usr/sbin/zonecfg -z ' + zone + ' info zonepath',
								function(err, stdout, stderr) {

							if (stderr) {
								smfLog.error({zone: zone}, 'cannot get zonepath');
								eh.unknownZone(zone);
							}

							var zpath = stdout.replace(/^.*: /, '').trim();
							smfLog.debug("putting '" + zpath + "' in zonepath cache");
							req.params.cache.p.zpaths[zone] = zpath;
							return next();
						});
					...
					// This is going to be run through zlogin. Do we need to use
					// pfexec(1) too?
					//
...
accessZone: function(req, res, next) {
	...
	// If we have an access file, look to see if the requested zone is
	// blocked. If
	...
	if (req.params.zone) {
		zone = (req.params.zone === '@') ? zone = req.params.cache.p.zonename :
			req.params.zone;
	}
	else {
		return next();
	...
	// Have we loaded in an access file?

	if (smfConfig.access) {
		
		// Is this zone blocked in that file? If there's no specific data for
		// this zone, fall back to the default.
		
		if (smfConfig.access[zone]) {

			if (smfConfig.access[zone]['default_action'] === 'block') {
				block = true;
			}

		}
		else if (smfConfig.access.default &&
						 smfConfig.access.default['default_action'] === 'block') {
			block = true;
		}
